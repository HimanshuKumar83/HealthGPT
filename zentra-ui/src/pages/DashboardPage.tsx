...
export default function DashboardPage() {
    ...
    useEffect(() => {
        chatApi.listSessions().then((res) => {
            ...
            const saved = localStorage.getItem("healthgpt_active_session");
    ...
    const loadSession = async (sessionId: string) => {
        ...
        localStorage.setItem("healthgpt_active_session", sessionId);
        setMessages([]);
    ...
    const createSession = async () => {
        try {
            ...
            localStorage.setItem("healthgpt_active_session", newSession.id);
            setMessages([]);
            setSidebarOpen(false);
        } catch { /* ignore */ }
    };
    ...
    const deleteSession = async (sessionId: string) => {
        try {
            ...
            if (activeSessionId === sessionId) {
                ...
                setMessages([]);
                localStorage.removeItem("healthgpt_active_session");
            }
        } catch { /* ignore */ }
    };
    ...
    const sendMessage = async () => {
        ...
        try {
            let fullContent = "";
            await chatApi.streamMessage(activeSessionId, text, (chunk) => {
                fullContent += chunk;
                setMessages((prev) =>
                    prev.map((m) =>
                        m.id === aiPlaceholderId ? { ...m, content: fullContent } : m
                    )
                );
            });
            ...
            setMessages((prev) =>
                prev.map((m) =>
                    m.id === aiPlaceholderId
                        ? { ...m, content: "Sorry, I couldn't reach the server. Please try again." }
                        : m
                )
            );
        } finally {
            ...
            >
                ...
                <div className="flex items-center justify-between px-4 h-14 border-b border-border shrink-0">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-sm tracking-tight">HealthGPT</span>
                    </div>
                    <button
                        onClick={() => setSidebarOpen(false)}
                        ...
                        {activeView === "results" ? "My Results"
                            : activeView === "profile" ? "Health Profile"
                                : activeView === "account" ? "My Profile"
                                    : (activeSession?.title ?? "HealthGPT AI")}
                    </span>
                </header>
                ...
                {activeView === "chat" && <div className="px-3 sm:px-6 py-3 sm:py-4 border-t border-border shrink-0">
                    <div className="w-full max-w-5xl mx-auto">
                        ...
                        <p className="text-xs text-muted-foreground text-center mt-2">
                            HealthGPT may make mistakes. Verify important health information with a professional.
                        </p>
                    </div>
                </div>}
            </div>
